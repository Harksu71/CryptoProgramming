...
char Feistel_Enc(char in)
{
	...
	left = (in >> 4) & 0x0F;	// 왼쪽 4bit
	right = in & 0x0F;			// 오른쪽 4bit
	...
	return (left << 4) | right;		// 4bit left와 right를 합쳐 8bit를 만듬
}
